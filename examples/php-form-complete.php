<?php
require_once __DIR__ . '/../php/EthiopianCalendar.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['name'])) {
    ...
    // Parse and format dates if available
    if (!empty($_POST['gregorian_date'])) {
        $ethDate = $calendar->toEthiopian($_POST['gregorian_date']);
        $formattedEth = $calendar->format($ethDate, 'd M y');
        
        $message = "
            <div class='result'>
                <h3>Form Submitted Successfully!</h3>
                <p><strong>Name:</strong> $name</p>
                <p><strong>Birth Date (Ethiopian):</strong> $formattedEth</p>
                <p><strong>Birth Date (Gregorian):</strong> $gregorianDate</p>
            </div>
        ";
    }
}
...
$todayEth = $calendar->now();
$todayFormatted = $calendar->format($todayEth, 'd M y');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="info">
        ...
        Ethiopian: <?php echo $todayFormatted; ?><br>
        Gregorian: <?php echo date('F j, Y'); ?>
    </div>
    ...
    <form method="POST" action="">
        ...
        <div class="form-group">
            <label for="birthdate">Birth Date (Click to select from Ethiopian Calendar):</label>
            <input type="text" 
                   id="birthdate" 
                   name="ethiopian_date" 
